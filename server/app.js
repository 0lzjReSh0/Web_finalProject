...
app.use(expressJWT.expressjwt({ secret: process.env.SECRET, algorithms: ["HS256"] }).unless({ path: ['/api/user/login', '/api/user/register', "/api/post", "/api/posts/comments"] }));
...
users.pre('save', async function (next) {
    ...
    const existingAdmin = await this.constructor.findOne({ email: "admin@163.com" });
...
app.get('/api/post', (req, res) => {
    ...
    try {
        ...
        Post.find().limit(limit).skip(skip).sort('-createdAt').then(posts => {
            // console.log(posts);
            res.json(posts)
        })
...
app.post('/api/posts/comments', (req, res) => {
    ...
    console.log(postId);
    try {
        Comment.find({ postId }).sort('-createdAt').then(comments => {
